...
abstract class RestController[TModel <: IMongoModel]
  ...
  def get(id: String) = Action { request =>
    Async {
      repository.get(new BSONObjectID(id)) map { _ match {
        case Some(model) =>
  ...
  def create = Action(parse.json) { request =>
    request.body.asOpt[TModel] match {
      case Some(model) =>
        if (model.isValid) {
          Async {
            repository.create(model) map { _ match {
  ...
  def remove(id: String) = Action {
    Async {
      repository.remove(new BSONObjectID(id)) map { ok =>
        if (ok) Status(204)
  ...
  def search = Action(parse.json) { request =>
    ...
    Async {
      repository.search(criteria) map { results =>
        val enumerator =
  ...
  private def modelToSerializedJsArray(enumerator: Enumerator[TModel]) = {
    ...
    enumerator.through(Enumeratee.map[TModel] { model =>
